<?php
/*
discription: send this file to client browser
parameter:
	filepath: full file path
	filename: filename string in http header
	filetype: 'xls' for example
	attachemnt: ture=>attachment,false=>inline
	unlink:true=>unlink filepath
*/
function passfile($filepath,$filename,$filetype,$attachment,$unlink=true){
	$MIMETypes = array(
   ...
   'iges'       => 'model/iges',
	...
	if(!is_file($filepath) || !is_readable($filepath)){
	 ...
	 /*
        ...
        */
	$contentType = (isset($MIMETypes[$filetype]))?$MIMETypes[$filetype]:'text/plain';

        header('Content-type: ' . $contentType);

	/*
         HTTP头信息: 如果为attachment, 则告诉浏览器, 在访问时弹出"文件下载"对话框, 并指定保存时文件的默认名称(可以与服务器的文件名不
同)
         如果要让浏览器直接显示内容, 则要指定为inline, 如图片, 文本
        */
	$position = ($attachment === true)?'attachment':'inline';
        header('Content-Disposition: ' . $position . '; filename=' . $filename);
        /*
         HTTP头信息: 告诉浏览器文件长度
         (IE下载文件的时候不是有文件大小信息么?)
        */
